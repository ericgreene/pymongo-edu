...
score for each student from the dataset that you imported in HW 2.1. Since each document is one grade,
...
To verify that you have completed this task correctly, provide the identity of the student
...
def remove_lowest_score(db):
    student_id = 0

    grades = db.grades.find().sort([("student_id", 1), ("score", 1)])
    db.grades.remove({'_id': grades.next()['_id']})
